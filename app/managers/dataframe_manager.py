...
from app.consts import DATETIME, EXCEL_DAY, UTC, EXCEL_WIND_DIRECTION, TIME_GAP, T, EXCEL_FF, TEMPERATURE, \
    WIND_DIRECTION_ID, WIND_SPEED, PAD, DATE, TIME, REGION
from app.formatters.wind_direction_formatter import parse_wind_direction
from app.managers.file_manager import get_date_from_path, get_files_paths, get_name_from_path, get_dir_from_path
...
def insert_into_dataframe_datetime_rows(dataframe: DataFrame, row: int, time_gap: str) -> tuple[DataFrame, int]:
    ...
    for i in range(len(date_times)):
        dataframe = pd.DataFrame(
            np.insert(dataframe.values, row + 1 + i,
                      values=[date_times[i], None, None, None, dataframe.iloc[0, -1]], axis=0))
...
def get_weather_api(dataframes: list[DataFrame], paths: list[str]) -> DataFrame:
    ...
    for i in range(len(dataframes)):
        region = get_dir_from_path(paths[i])
        weathers_api.append(
            create_weather_dataframe(dataframes[i],
                                     get_date_from_path(paths[i], 2),
                                     get_date_from_path(paths[i], 1),
                                     region)
        )
    return merge_sort_dataframe_list(weathers_api, DATETIME)


def create_weather_dataframe(dataframe: DataFrame, month: int, year: int, region: str) -> DataFrame:
    rows = []
