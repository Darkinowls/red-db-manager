...
from app.consts import WIND_TABLE, INT, WIND_DIRECTION_ID, ID, NAME, ANGLE
from app.managers.database_manager import get_dataframe_from_table

wind_dataframe = get_dataframe_from_table(WIND_TABLE)
wind_directions_dictionary = tuple(
    zip(wind_dataframe[ID], wind_dataframe[NAME], wind_dataframe["name_rus"], wind_dataframe['angle']))
...
def translate_wind_direction(dataframe: DataFrame, lang: int):
    df = dataframe[WIND_DIRECTION_ID]
    df = df.apply(parse_wind_direction, args=(lang,))
    return df
